...
class Processor(lmfit.Parameters):
        ...
        for i, func_info_dic in enumerate(self.func_info_lst):
            ...
            if func_info_dic['function'] == 'lorentzian':
                for name in ['amplitude', 'center']:
                    self.lmfit_params.add(name='l'+str(i)+'_'+name,
                                          **func_info_dic[name])
                for name in ['sigma']:
                    if same_idx is None:
    ...
    def set_function_for_optmization(self):
        ...
        for i in range(len(self.func_info_lst)):
            if self.func_info_lst[i]['function'] == 'lorentzian':
                loren = lmfit.models.LorentzianModel(prefix='l'+str(i)+'_')
                func_lst.append(loren)
            else:
                raise ValueError("what is %s" % self.func_info_lst[i]['function'])
        self.optimize_function = sum(func_lst)


    def fit(self):
        # out = lmfit.minimize(self.optimize_function, self.lmfit_params,
        #                  args=(self.x_arr, self.y_arr))
        out = self.optimize_function(self.y_arr, self.lmfit_params, x=self.x_arr)
        return out
